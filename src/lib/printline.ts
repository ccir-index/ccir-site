...
// published the next morning, so the page shows both: the day the print
// applies to and when it was published. No label word (John, 2026-09-17:
// "fixing day" is not chart vocabulary); the print date stands alone. Without the second half a reader on the 17th
...
export function printLine(meta: { as_of_date: string; published_at_ct?: string }): string {
  ...
  return pub ? `${meta.as_of_date} print · published ${pub}` : `${meta.as_of_date} print`;
}
